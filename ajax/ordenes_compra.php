...
require_once "../modelos/Ordenes_compra.php";

$ordenes_compra = new Ordenes_compra();

$idorden_compra = isset($_POST["idorden_compra"]) ? limpiarCadena($_POST["idorden_compra"]) : "";
...
$estado_orden_compra = isset($_POST["estado_orden_compra"]) ? limpiarCadena($_POST["estado_orden_compra"]) : "";
...
switch ($_GET["op"]) {
    case 'guardaryeditar':
        ...
        if (empty($idorden_compra)) {
            $rspta = $ordenes_compra->insertar(
                $idcliente,
                $codigo_cliente,
                $nit,
                $nombre_cliente,
            ...
            echo $rspta ? "Orden Compra registrado" : "No se pudieron registrar todos los datos del Orden Compra";
        } else {
            $rspta = $ordenes_compra->editar(
                $idorden_compra,
                $idcliente,
                $codigo_cliente,
                $nit,
            ...
            echo $rspta ? "Orden Compra actualizada" : "Orden Compra no se pudo actualizar";
        }
        break;

    case 'guardaryeditar_ingreso_orden_compra':
        if (isset($_POST['datosArticulosC'])) {
            $datosArticulos = json_decode($_POST['datosArticulosC'], true);
        }
        if (empty($idorden_compra)) {
            /*  $rspta=$ordenes_compra->insertar($idcliente,$codigo_cliente,$nit,$nombre_cliente,$telefono_cliente,$direccion_cliente,$correo_cliente,$tipo_documento_cliente,$idusuario,$tipo_comprobante,
            ...
            $fecha_entrega_orden_compra,$observacion_orden_compra,$tipo_ingreso_producion,
            
            $datosArticulos,$total_compra_r,$total_comprades_r
        
            );
            echo $rspta ? "Orden Compra registrado" : "No se pudieron registrar todos los datos del Orden Compra";
        */
        } else {
            $rspta = $ordenes_compra->editar_ingreso_orden_compra(
                $idorden_compra,
                $idcliente,
                $codigo_cliente,
                $nit,
    ...
    case 'guardaryeditarND':
        ...
        $rspta = $ordenes_compra->insetar_nd(
            $idorden_compra,
            $idcliente,
            $codigo_cliente,
            $nit,
    ...
    case 'anular':
        $rspta = $ordenes_compra->anular($idorden_compra);
    ...
    case 'mostrar':
        $rspta = $ordenes_compra->mostrar($idorden_compra);
    ...
    case 'detalleingreso':
        $cotiz = $ordenes_compra->detalleingreso($idorden_compra);
    ...
    case 'listar':
        ...
        $rspta = $ordenes_compra->listar($fecha_inicio_reporte, $fecha_fin_reporte);
        //Vamos a declarar un array 
        $data = array();

        while ($reg = $rspta->fetch_object()) {
            $url = '../reportes/exOrden_compra.php?id=';
            $data[] = array(
                "0" => ($reg->estado == 'Aceptado') ? '<button class="btn btn-warning" onclick="mostrar(' . $reg->idorden_compra . ')"><i class="fa fa-eye"></i></button>' .
                    ' <button class="btn btn-danger" onclick="anular(' . $reg->idorden_compra . ')"><i class="fa fa-close"></i></button>' . '<a target="_blank" href="' . $url . $reg->idorden_compra . '"><button class="btn btn-info"><i class="fa fa-print"></i> </button> </a>' : '<a target="_blank" href="' . $url . $reg->idorden_compra . '"><button class="btn btn-info"><i class="fa fa-print"></i> </button> </a>',
                "1" => $reg->idorden_compra,
                "2" => $reg->fecha,
                "3" => $reg->proveedor,
                "4" => $reg->usuario,
                ...
                "9" => ($reg->estado_compra == 'Orden de compra') ? '<span class="label bg-yellow">Orden de compra</span>' :
                    '<span class="label bg-green">Ingresar Orden de Compra</span>',
            );
        }
        $results = array(
    ...
    case 'listar_ingreso_orden_compra':
        $fecha_inicio_reporte = $_REQUEST["fecha_inicio_reporte"];
        $fecha_fin_reporte = $_REQUEST["fecha_fin_reporte"];
        $rspta = $ordenes_compra->listar($fecha_inicio_reporte, $fecha_fin_reporte);
        //Vamos a declarar un array 
        ...
        while ($reg = $rspta->fetch_object()) {
            ...
            $data[] = array(
                "0" => ($reg->estado == 'Aceptado') ? '<button class="btn btn-warning" onclick="mostrar(' . $reg->idorden_compra . ')"><i class="fa fa-eye"></i></button>' :
                    '<a target="_blank" href="' . $url . $reg->idorden_compra . '"><button class="btn btn-info"><i class="fa fa-print"></i> </button> </a>',
                "1" => $reg->idorden_compra,
                "2" => $reg->fecha,
                "3" => $reg->proveedor,
                "4" => $reg->usuario,
                ...
                "7" => ($reg->estado == 'Aceptado') ? '<span class="label bg-green">Aceptado</span>' :
                    '<span class="label bg-red">Anulado</span>',
                "8" => ($reg->estado_compra == 'Orden de compra') ? '<span class="label bg-yellow">Orden de compra</span>' :
                    '<span class="label bg-green">Ingresar Orden de Compra</span>',
            );
        }
        $results = array(
    ...
    case 'listarND':
        $fecha_inicio_reporte = $_REQUEST["fecha_inicio_reporte"];
        $fecha_fin_reporte = $_REQUEST["fecha_fin_reporte"];
        $rspta = $ordenes_compra->listarND($fecha_inicio_reporte, $fecha_fin_reporte);
        //Vamos a declarar un array 
        $data = array();
        ...
        while ($reg = $rspta->fetch_object()) {
            ...
            $data[] = array(
                "0" => '<a target="_blank" href="' . $url . $reg->idnota_debito . '"><button class="btn btn-info"><i class="fa fa-tag"></i> </button> </a>',
                "1" => $reg->idnota_debito,
                "2" => $reg->fechand,
                "3" => $reg->idorden_compra,
                "4" => $reg->fecha,
                "5" => $reg->proveedor,
                "6" => $reg->usuario,
            ...
            $data[] = array(
                "0" => '<div class="input-group" style="display: inline-flex; margin-right: 5px;">
                        ...
                        <button class="btn btn-warning" onclick="agregarDetalleCanta(' . $reg->idarticulo . ',
                                                                                ...
                                                                                \'' . $reg->precio_venta . '\',
                                                                                \'' . $reg->precio_compra . '\',
                                                                                \'' . $reg->stock . '\',\'' . $reg->precio_ventaNocturno . '\',
                                                                                ...
                                                                                \'' . $reg->idsucursal . '\',\'' . $reg->nom_sucursal . '\',
    ...
    case 'selectSucursal':
        ...
        $rspta = $ordenes_compra->selectSucursal();
