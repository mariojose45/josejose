...
switch ($_GET["op"]){
    ...
    case 'listar':
        ...
        while ($reg=$rspta->fetch_object()){
            ...
            $data[]=array(
                "0"=>
                '<a target="_blank" href="'.$url.$reg->id.'"><button class="btn btn-info"><i class="fa fa-file"></i> </button> </a>',
                "1"=>$reg->fecha,
                "2"=>$reg->id,
                "3"=>$reg->total_venta,
    ...
    case 'listarArticulosVentacajachica':
        require_once "../modelos/Articulo.php";
        $articulo=new Articulo(); 
  
        $rspta=$articulo->listarActivosCajachica();
        //Vamos a declarar un array
        $data= Array();
