...
function init(){
    ...
    {   
         ...
         var ressaldocuenta=$("#idcuenta23 option:selected").attr("data-saldo_cuenta");

         $("#saldo_cuenta").val(ressaldocuenta);
...
function limpiar()
{
    ...
    $('#fecha_hora_factura').val(today);
    $('#fecha_hora_operacion').val(today);
...
{
    bootbox.confirm("¿Está Seguro de Anular El cheque?", function(result){
        if(result)
        {
