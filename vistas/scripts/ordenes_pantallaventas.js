...
function procesarDetalles(detalles) {
    ...
    detalles.forEach(det => {
        ...
        } else if (det.tipo === 'Extra' || det.tipo === 'Topping') {
            if (articulosPadre.length > 0) {
                const ultimoPadre = articulosPadre[articulosPadre.length - 1];
                ultimoPadre.hijos.push(det);
            }
        }
    });
...
function listar() {
    ...
    $.ajax({
        url: "../ajax/venta.php?op=listarpantallaventas",
        type: "GET",
                ...
                <div class="col-md-3 col-sm-6">
                    <div class="orden-card card mb-2">
                        <div class="orden-header card-header bg-light p-2">
                            <small><b>Venta # Interno${item.idventa}</b></small>
                        </div>
                    ...
                    let filaPadre = `
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <small class="text-muted" style="font-size: 1.1em;">${padre.descripcion_detalle ? `(${padre.descripcion_detalle})` : ''}</small>
                                            </td>
                                        </tr>
                                    `;
                    ...
                    let filasHijas = padre.hijos.map(hijo => `
                                        <tr>
                                            ...
                                            <td style="padding-left: 20px;">
                                                ...
                                                | <span class="badge badge-info float-right" style="font-size: 85%;">${hijo.tipo.toUpperCase()}</span>
                                            </td>
                                        </tr>
                                    `).join("");
                    ...
                    return filaPadre + filasHijas;
                        ...
                        <div class="card-footer text-center">
                            <button class="btn btn-success btn-block btn-listo" 
                                onclick="listo(${item.idventa}, this)">
...
function listo(idventa) {
    bootbox.confirm("¿Está Seguro de marcar como listo tu pedido?", function (result) {
        if (result) {
            $.post("../ajax/venta.php?op=listo", { idventa: idventa }, function (e) {
                bootbox.alert(e);
                tabla.ajax.reload();
            });
        }
