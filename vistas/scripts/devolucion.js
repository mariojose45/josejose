...
{
    $.post("../ajax/devolucion.php?op=mostrar", { idcotizacion: idcotizacion }, function (data, status) {
          
        
        // Parseamos la data
        data = JSON.parse(data); 

        console.log(data.idnota_credito)

        // Validar si idnota_credito es distinto de 0
        if (!data || data.idnota_credito != 0) { 
            // Aquí se valida si no existe `data` o si `idnota_credito` no es igual a 0
            Swal.fire({
                icon: 'info',
                title: 'Venta ya tiene NC',
                text: 'La Venta ya fue aplicado una Nota de Crédito (NC) y no está disponible.',
                confirmButtonText: 'Aceptar'
            });
            return; // Detenemos la ejecución si no cumple la validación
        }
        load(); 
...
function guardaryeditar(e)
{
    ...
    const cNotaCredito = parseFloat($("#cNotaCredito").val().trim());        
     ...
     if(forma_pago == "Efectivo")
    {
        ...
        if (isNaN(cNotaCredito) || cNotaCredito <= 0) {
            Swal.fire({
                icon: "error",
                title: "Oops...",
                text: "Ingresa el valor en Nota Creddto no lo puedes dejar en blanco ni con valor negativo",
                footer: '<a >Venta.</a>'
            });
            return;
        }
...
{
    $.post("../ajax/devolucion.php?op=paraventa",{idcotizacion:idcotizacion},function(data){
        //console.log(data);
        data = JSON.parse(data);          
        Swal.close() 
...
function agregarDetalle2(idarticulo,nombre,precio_venta,stock,descuento,stock_unidad,precio_unidad,
                        ...
                        cantidad,presen,cantidadpresentacion,totalcantidadpresentacion,precio_ventaSistema,
                        precio_ventaSistema2,q_ref,precio_recargoPV,precio_recargoQRef,precio_rango1,precio_rango2,precio_rango3)
{
    ...
    { 
        ...
        if(!exist) 
        {
            ...
            '<td><input type="hidden" name="stockinven[]" value="'+stockinven+'">'+stockinven+'</td>'+
            '<td><input style="width:60px" type="hidden" id="cantidadpresentacion'+cont+'" name="cantidadpresentacion[]" value="'+cantidadpresentacion+'" onchange="modificarSubototales()"><input style="width:60px" class="form-control"  onchange="modificarSubototalesxrango('+cont+','+precio_rango1+','+precio_rango2+','+precio_rango3+',this)"  type="number" step="any"   id="cxcantidad'+idarticulo+'" name="cantidad[]" id="cantidad'+cont+'" value="'+cantidad+'"><input style="width:60px"  type="hidden" id="totalcantidadpresentacion'+cont+'" name="totalcantidadpresentacion[]" value="'+totalcantidadpresentacion+'" onchange="modificarSubototales()"></td>'+
            '<td><input style="width:100px" type="text"  readonly="" class="form-control" name="presentacion[]" id="presentacionselect'+cont+'"  value="'+presen+'" onchange="modificarSubototales()">'+
            `<td><input  type="hidden"  name="presen[]" id="presen`+cont+`"value="`+presen+`" ">
                <input type="hidden" name="precio_ventaSistema[]" id="precio_ventaSistema`+cont+`" value="`+precio_venta+`">
                <input type="hidden" name="precio_ventaSistema2[]" id="precio_ventaSistema2`+cont+`" value="`+precio_venta+`">
                <input class="form-control" style="width:100px" type="number" step="any" name="q_ref[]" id="q_ref`+cont+`" value="`+precio_venta+`"  readonly >
