...
function init() {
    ...
    $.post("../ajax/ordenes_compra.php?op=selectSucursal", function (r) {
        ...
        $("#idsucursal").html(options).selectpicker('refresh');
    });
    ...
    tabla = $('#tbllistado').dataTable(
        {
            ...
            "ajax":
            {
                url: '../ajax/ordenes_compra.php?op=listar',
                type: "get",
...
function guardaryeditar(e) {
    ...
    if (detalles > 0) {
        ...
        alert("No se puede Guardar porque no has agregado items ");
        return;
    }
    agruparDatos();
    ...
    load();
    $.ajax({
        url: "../ajax/ordenes_compra.php?op=guardaryeditar",
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
...
function agregarDetalleCanta(idarticulo, articulo, precio_venta, precio_compra, stock, precio_ventaNocturno,
    ...
    cantidad) {
    //Si la cantidad es 0 o null, se establece en 1
    cantidad = (cantidad == 0 || cantidad == null) ? 1 : cantidad;
    var subtotaldes = 0;
    ...
    if (idarticulo != "") {
        ...
        var subtotal = parseFloat(cantidad) * parseFloat(precio_compra);
        var fila = '<tr class="filas" id="fila' + cont + '">' +
            ...
            '<td><input style="width:60px" type="hidden" id="cantidadpresentacion' + cont + '" name="cantidadpresentacion[]" value="' + cantidadpresentacion + '" ><input  class="form-control"  onchange="modificarSubototales()"  type="number" step="any"   id="cxcantidad' + idarticulo + '" name="cantidad[]" id="cantidad' + cont + '" value="' + cantidad + '" style="width:100px"><input style="width:60px"  type="hidden" id="totalcantidadpresentacion' + cont + '" name="totalcantidadpresentacion[]" value="' + totalcantidadpresentacion + '"></td>' +
            `<td>
    ...
    $.post("../ajax/ordenes_compra.php?op=detalleingreso", { idorden_compra: idorden_compra }, function (data) {
        ...
        $.each(data, function (i, item) {
            agregarDetalle2(item.idarticulo, item.articulo, item.precio_venta, item.precio_compra, item.stock, item.cantidad,
                ...
                item.nombre_20, item.stock_20, item.precio_20, item.facturar_cero, item.idsucursalDestino, item.sucursalDestino
            );
        });
    })
}
...
function agregarDetalle2(idarticulo, articulo, precio_venta, precio_compra, stock, cantidad,
    ...
    nombre_18, stock_18, precio_18,
    nombre_19, stock_19, precio_19,
    nombre_20, stock_20, precio_20, facturar_cero, idsucursalDestino, sucursalDestino) {
...
function modificarSubototales() {
    ...
    for (var i = 0; i < cant.length; i++) {
        ...
        inpS.value = ((inpCpre.value * inpC.value) * (inpP.value - ((inpP.value * inpD.value) / 100)));
...
function calcularTotales() {
    ...
    for (var i = 0; i < sub.length; i++) {
        total += parseFloat(sub[i].textContent) || 0;
    }
