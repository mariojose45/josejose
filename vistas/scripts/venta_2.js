...
function obtenerClienteCotizacion(idcotizacion) 
{
    ...
    $.post("../ajax/toma_orden.php?op=mostrarCobro", { idcotizacion: idcotizacion }, function (data, status) {
         
        // Parseamos la data
        data = JSON.parse(data); 

        // Verificamos si la data viene vacía o es nula
        if (!data || !data.id_add_orden) {
            Swal.fire({
                icon: 'info',
                title: 'Orden cobrada',
                text: 'La Orden ya fue cobrada o no está disponible.',
...

{
    ...
    tabla=$('#tbllistado').dataTable(
    {
        ...
        "ajax":
        {
            url: '../ajax/venta.php?op=listarRestaurante',
    ...
    if(forma_pago == "Credito")
    {
        ...
        if (isNaN(cefectivoTransferencia) || cefectivoTransferencia > 0) {
            Swal.fire({
                icon: "error",
                title: "Oops...",
                text: "No puedes dejar vacio Transferencia, no puede ser mayor a 0 ",
                footer: '<a >Venta.</a>'
            });
            return;
        }
        ...
        if (isNaN(cefectivo) || cefectivo > 0) {
            Swal.fire({
                icon: "error",
                title: "Oops...",
                text: "No puedes dejar vacio Efectivo, no puede ser mayor a 0",
                footer: '<a >Venta.</a>'
            });
            return;
        }
        ...
        if (isNaN(cefectivo_tarjeta) || cefectivo_tarjeta > 0) {
            Swal.fire({
                icon: "error",
                title: "Oops...",
                text: "No puedes dejar vacio Tarjeta, no puede ser mayor a 0 ",
                footer: '<a >Venta.</a>'
            });
            return;
        }              
    ...
    else if(forma_pago == "Efectivo")
    {
        ...
        if (isNaN(cefectivoCredito)  || cefectivoCredito > 0) {
            Swal.fire({
                icon: "error",
                title: "Oops...",
                text: "No puedes dejar vacio Credito, no puede ser mayor a 0  ",
                footer: '<a >Venta.</a>'
            });
            return;
        }
    ...
    { 
        if (parseFloat(totalventa)>=2500) 
        {
            if (numdocumentocliente!='CF' && numdocumentocliente!='C/F' && numdocumentocliente!='cf' && numdocumentocliente!='c/f')  
            {
                ...
                load();    
                $.ajax({
                    url: "../ajax/venta.php?op=guardaryeditarCobroOrden", 
                    type: "POST",
                    data: formData,
                    contentType: false,
                    processData: false, 
    ...
    stock_blister,precio_blister,stock_caja,precio_caja,stock_fardo,precio_fardo,stock_sacos,precio_sacos,stock_paquete,precio_paquete,precio_rango1,precio_rango2,precio_rango3)
{
    ...
    { 
        ...
        if(!exist) 
        {
            ...
            '<td><input type="hidden" name="valor_descuentoGeneralLista[]" id="valor_descuentoGeneralLista' + cont + '" value="0"><input type="hidden" name="descripcion_detalle[]" value="."><input type="hidden" name="stockinven[]" value="'+stockinven+'">'+stockinven+'</td>'+
...
{
    $.post("../ajax/toma_orden.php?op=detalleorden2",{idcotizacion:idcotizacion},function(data){
        //console.log(data);
        data = JSON.parse(data);          
        Swal.close() 
...
function calcularTotales() {
     var propina=$("#propina").val(); var propina=$("#propina").val();
    var sub = document.getElementsByName("subtotal");
    var total = 0.0;
