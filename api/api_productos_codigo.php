...
// 🔹 Recibir datos desde Flutter (JSON)
$data = json_decode(file_get_contents("php://input"), true);

$codigo = isset($data['codigo']) ? trim($data['codigo']) : '';
$idsucursal = isset($data['idsucursal']) ? intval($data['idsucursal']) : 0;

if (empty($codigo) || $idsucursal <= 0) {
    http_response_code(400);
    echo json_encode(["success" => false, "message" => "Parámetros inválidos (requiere código e idsucursal)"]);
    exit;
}
...
// Modificar tu método o crear otro que reciba $codigo y $idsucursal
$rspta = $articulo->ObtenerProductoBarCode2Api($codigo, $idsucursal);
