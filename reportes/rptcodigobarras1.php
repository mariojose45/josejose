...
$nombre = (strlen($nombre) > 20) ? substr($nombre, 0, 17) . '...' : $nombre;
    ...
    $stepY = 30; // Incremento en Y entre cada fila
    $barcodeWidth = 30;
    $barcodeHeight = 15;

    // Generar las filas y columnas dinámicamente
    for ($row = 0; $row < 8; $row++) { // 19 filas
        for ($col = 0; $col < count($columns); $col++) { // 4 columnas
            $currentX = $columns[$col];
            $currentY = $startY + ($row * $stepY); // Calcular Y dinámico
            
            // Generar código de barras con la nueva altura
            $pdf->Code128($currentX, $currentY, $code, $barcodeWidth, $barcodeHeight);

            // Añadir texto del código (ahora con mayor espacio abajo)
            $pdf->SetXY($currentX, $currentY + $barcodeHeight + 2);  // Aumentamos el desplazamiento en Y
            $pdf->Cell($barcodeWidth, 4, $code, 0, 0, 'C');

            // Añadir nombre del producto (ahora con más espacio después del código)
            $pdf->SetXY($currentX, $currentY + $barcodeHeight + 6);  // Aumentamos el desplazamiento en Y
            $pdf->Cell($barcodeWidth, 4, $nombre, 0, 0, 'C');

            // Añadir precio de venta (más espacio para que no se solape con el nombre)
            $pdf->SetXY($currentX, $currentY + $barcodeHeight + 10);  // Aumentamos el desplazamiento en Y
