...
//$pdf->addSociete($logo,$ext_logo);
    ...
    $pdf->fact_dev( utf8_decode("NOTA CREDITO"),"" );
    $pdf->temporaire( "" ); 
...
//$pdf->Rect(10,43, 100, 4, 'F'); //Rectángulo relleno y con liena
    ...
    $pdf->Ln(1);
    $pdf->SetX($xposision); 
    $pdf->Multicell($ancchodefial,4,utf8_decode("Fecha Emision: ".date("d/m/Y", strtotime($regv->fecha_hora_nc))),0,"C"); 
    ...
    $pdf->Ln(5);
    $pdf->SetX($xposision);     
    $pdf->Multicell($ancchodefial,4,utf8_decode("DOCUMENTOS DE REFERENCIA VENTA "),0,"C");  
    
    $pdf->Ln(5);
    $pdf->SetX($xposision);     
    $pdf->Multicell($ancchodefial,4,utf8_decode("NUMERO DE AUTORIZACION "),0,"C");                         
    $pdf->Ln(2);
    $pdf->SetX($xposision);     
    $pdf->Multicell($ancchodefial,4,utf8_decode($regv->autorizacionEcoFactura_venta),0,"C");     
    $pdf->Ln(5);
    $pdf->SetX($xposision);     
    $pdf->Multicell($ancchodefial,4,utf8_decode("SERIE: ".$regv->serie_comprobante_venta),0,"C");
    $pdf->Ln(5);
    $pdf->SetX($xposision);     
    $pdf->Multicell($ancchodefial,4,utf8_decode("NUMERO: ".$regv->numero_ecoFactura_venta),0,"C");                                     
    $pdf->Ln(5);
    $pdf->SetX($xposision);     
    $pdf->Multicell($ancchodefial,4,utf8_decode("FECHA EMISION VENTA: "),0,"C");                         
    $pdf->Ln(5);
    $pdf->SetX($xposision);     
    $pdf->Multicell($ancchodefial,4,utf8_decode(date("d/m/Y", strtotime($regv->fechaVenta))),0,"C");  

    $pdf->Ln(5);
    $pdf->SetX($xposision);     
    $pdf->Multicell($ancchodefial,4,utf8_decode("MOVITVO NC: ".$regv->motivo_nc),0,"C");         

    $pdf->Ln(5);
    $pdf->SetX($xposision);     
    $pdf->Cell($ancchodefial,4,utf8_decode("IDVENTA: ".$regv->idventa),0,0,"C");           
    ...
    $pdf->Output('NC Dte No '.$regv->numero_ecoFactura.".pdf",'I');
