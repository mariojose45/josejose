...
/////PARTE DE ABAJO            
            $pdf->fact_dev( utf8_decode("COTIZACION NO: ".$regv->num_comprobante),"" );
            $pdf->temporaire( "" ); 
            $pdf->addDate3( ""," "," ","# Interno: ".$regv->idcotizacion,$regv->fecha);
            $pdf->SetXY(110,1); 
            $pdf->Multicell(100,4,utf8_decode("COTIZACION DE PRODUCTOS"),0,"C"); 
        ...
        //$pdf->Rect(10,43, 100, 4, 'F'); //Rectángulo relleno y con liena
            ...
            $pdf->SetXY(30,209);
            $pdf->Image($url.$regv->sucursal_imagen,10 ,3, 60 , 15 );
            $pdf->SetFont('Arial','',8);
            $pdf->SetXY(10,20); 
            ...
            $pdf->SetXY(50,23); 
            $pdf->Cell(70,4,"Nit: ".$regv->sucursal_nit,0,0,"L");     
            $pdf->SetXY(10,27); 
...
/////PARTE DE ARRIBA        
    ...
    $pdf->SetTextColor(0,0,0); 
     $cantidad=0;
        ...
        $rsptadExtra = $cotizaciones->cotizaciondetalleExtra($_GET["id"]);
        ...
        while ($regdExtra = $rsptadExtra->fetch_object()) {
            $precio = 0;
            $sub_total = 0;
            if($regdExtra->tipo_item == "Extra"){
                $precio = $regdExtra->precio_extra;
                $sub_total = $regdExtra->cantidad_extra * $regdExtra->precio_extra;
            }else{
                $precio = 0;
                $sub_total = 0;
            }
          ...
          $line = array("CANT"=> number_format($regdExtra->cantidad_extra, 2, '.', ','),
                        "DESCRIPCION"=> utf8_decode("$regdExtra->articulo"),
                         "CODIGO"=> "$regdExtra->codigo",
                        "P.U."=>  number_format($precio, 2, '.', ','),
        ...
        $pdf->Cell(190,5,".::ULTIMA LINEA::.",1,0,'C',1); 
        $pdf->SetTextColor(0,0,0); 
        $pdf->Output($regv->nombre_comercial.' - Cotizacion No'.$regv->idcotizacion.".pdf", 'I');
    } else {
