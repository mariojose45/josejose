...
function fact_dev($libelle, $num) {
    ...
    $r1  = $this->w - 58; // X position of the left side of the rectangle
    $r2  = $r1 + 58;       // X position of the right side of the rectangle
    $y1  = 30;             // Y position of the top of the rectangle
    $y2  = $y1 + 10;       // Y position of the bottom of the rectangle (increased height to fit text)
    $mid = ($r1 + $r2) / 2;
    ...
    $this->SetXY($r1 + 1, $y1 + 3); // Adjusted position to center text vertically
...
function addDate($idingreso, $fecha )
{
    $r1  = $this->w - 58; // X position of the left side of the rectangle
    $r2  = $r1 + 58;       // X position of the right side of the rectangle
    $y1  = 45;             // New Y position of the top of the rectangle
    $y2  = 15;              // Height of the rectangle
    $mid = $y1 + ($y2 / 3);
    ...
    $this->SetFont("Arial", "", 12);
    $this->SetXY(1, $y1 + 2); // Adjusted position for # Interno
    $this->Cell(70, 5, "# Interno: " . $idingreso, 0, 0, "L");
    $this->SetXY(1, $y1 + 6); // Adjusted position for Fecha Emision
    $this->Cell(70, 5, "Fecha Emision: " . $fecha, 0, 0, "L");
    
    // Adjust the Y position for the next text if needed
    $this->SetFont("Arial", "", 12);         
}
