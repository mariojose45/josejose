...
//Establecemos los datos de la empresa
    ...
    $tomaordenes= new TomaoOrdenes();

    $rsptav = $tomaordenes->cabeceraOrden($_GET["id"]); 
...
//$pdf->addSociete($logo,$ext_logo);
    ...
    $pdf->fact_dev( utf8_decode("ORDEN MESA"),"" );
    $pdf->temporaire( "" ); 
...
//$pdf->Rect(10,43, 100, 4, 'F'); //Rectángulo relleno y con liena
    ...
    $pdf->Ln(10);
    $pdf->SetX($xposision); 
    $resultadoDetalle = $tomaordenes->detalleOrdenMesa($_GET["id"]); 
    ...
    $pdf->Ln(5);
    $pdf->SetX($xposision);     
    $pdf->Cell($ancchodefial,4,utf8_decode("PROPINA: ".$regv->propina),0,0,"R"); 
    $pdf->Ln(5);
    $pdf->SetX($xposision);     
    $pdf->Cell($ancchodefial,4,utf8_decode("SUBTOTAL: ".$regv->total_general),0,0,"R"); 
    $pdf->Ln(5);
    $pdf->SetX($xposision);     
    ...
    $pdf->Ln(5);
    $pdf->SetX($xposision);     
    $pdf->Cell($ancchodefial,4,utf8_decode("# Orden: ".$regv->num_comprobante),0,0,"L");     

    $pdf->Ln(5);
    $pdf->SetX($xposision);     
    $pdf->Cell($ancchodefial,4,utf8_decode("# Interno: ".$regv->id_add_orden),0,0,"L");       
    ...
    $pdf->Output('Ordden Memsa '.$regv->id_add_orden.".pdf",'I');
