...
if (!isset($_SESSION["nombre"])) {
    ...
    if ($_SESSION['nomina_empleados'] == 1) {
        ...
        function comprobanteNominaMediaHoja($pdf, $reg, $regv, $posY)
        {
            ...
            if (file_exists($logo)) {
                //$pdf->Image($logo, 88, $posY - 15, 30);
            }

            // === NÚMERO DE NÓMINA ARRIBA A LA DERECHA ===
            $pdf->SetXY(125, $posY - 5);
            $pdf->SetFont('Arial','B',10);
            $pdf->Cell(75,5,utf8_decode("# Recibo: ".$regv->idnomina_pagos_14_aguinaldo." - ".$reg->iddetalle_nomina_pagos_14_aguinaldo),0,1,'R');

        
            // ===== TÍTULOS =====
            $pdf->SetXY(10, $posY + 10);
            $pdf->SetFont('Arial','B',12);
            $pdf->Cell(0,6,utf8_decode("Comprobante de pago: ".$regv->tipo_operacion),0,1,'C');
            ...
            $pdf->Cell(40,5,number_format($reg->dias_trabajados,0),0,0,'R');
            $pdf->Cell(50,5,utf8_decode("IGSS"),0,0);
            $pdf->Cell(40,5,number_format(0,2),0,1,'R');
            ...
            $pdf->Cell(50,5,utf8_decode($regv->tipo_operacion),0,0);
            $pdf->Cell(40,5,number_format($reg->salario_base,2),0,0,'R');
            ...
            $pdf->Cell(50,5,utf8_decode("Bonificación Productividad"),0,0);
            $pdf->Cell(40,5,number_format(0,2),0,0,'R');
            $pdf->Cell(50,5,utf8_decode("Otras Deducciones"),0,0);
            $pdf->Cell(40,5,number_format($reg->anticipos_bono_14,2),0,1,'R');
            ...
            $pdf->SetFont('Arial','B',9);
            $pdf->Cell(90,6,utf8_decode("Salario Bruto:"),0,0,'L');
            $pdf->Cell(90,6,number_format($reg->total_devengado,2),0,1,'R');
        
            $pdf->Cell(90,6,utf8_decode("Total de Deducciones:"),0,0,'L');
            $pdf->Cell(90,6,number_format($reg->anticipos_bono_14,2),0,1,'R');
