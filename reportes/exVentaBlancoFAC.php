...
//function addDate( $serie,$numero,$fechafac,$numAutorizacion,$fecha )
    ...
    $pdf->SetTextColor(0,0,0); 
     $cantidad=0;
        ...
        while ($regd = $rsptad->fetch_object())
        {
          ...
          $line = array("CANT"=> "$regd->cantidad",
                        "DESCRIPCION"=> utf8_decode("$regd->articulo $regd->descripcion_detalle"),
                         "CODIGO"=> "$regd->codigo",
                        "P.U."=>  "$regd->q_ref",
                        "SUBTOTAL"=> "$regd->totalsindescuento");
        ...
        $pdf->SetXY(10,255);      
        $pdf->Cell(95,4,utf8_decode("# Venta: ".$regv->num_comprobante),0,0,"C");                 
        $pdf->Cell(95,4,utf8_decode("Nº de venta: # ".$regv->num_comprobante),0,0,"C");                 
        $pdf->SetXY(10,260);    
