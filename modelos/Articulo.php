...
class Articulo
{
    ...
    public function listarActivosMateria()
    {
        $sql = "SELECT 
                ...
                a.aplica_comision,
                asu.idsucursal,
                ROUND(asu.stocksucursal, 6) AS stock,
                ROUND(asu.stockminimo, 6) AS stockminimo,
                ROUND(asu.precio_compra, 6) AS precio_compra,
                ROUND(asu.precio_venta, 6) AS precio_venta,
                ROUND(asu.precio_ventaNocturno, 6) AS precio_ventaNocturno,
                ROUND(asu.descuento_porcentaje, 6) AS descuento_porcentaje,
                ROUND(asu.precio_descuento, 6) AS precio_descuento,
                ROUND(asu.precio_rango1, 6) AS precio_rango1,
                ROUND(asu.precio_rango2, 6) AS precio_rango2,
                ROUND(asu.precio_rango3, 6) AS precio_rango3,
                ROUND(asu.stock_unidad, 6) AS stock_unidad,
                ROUND(asu.precio_unidad, 6) AS precio_unidad,
                ROUND(asu.stock_blister, 6) AS stock_blister,
                ROUND(asu.precio_blister, 6) AS precio_blister,
                ROUND(asu.stock_caja, 6) AS stock_caja,
                ROUND(asu.precio_caja, 6) AS precio_caja, 
                ROUND(asu.stock_fardo, 6) AS stock_fardo, 
                ROUND(asu.precio_fardo, 6) AS precio_fardo,
                ROUND(asu.stock_sacos, 6) AS stock_sacos,
                ROUND(asu.precio_sacos, 6) AS precio_sacos,
                ROUND(asu.stock_paquete, 6) AS stock_paquete,
                ROUND(asu.precio_paquete, 6) AS precio_paquete,
                asu.condicion,
                asu.descripcion_2
                FROM articulo a  
    ...
    public function mostrar($idarticulo)
    {
        $sql = "SELECT 
                ...
                ROUND(asu.stockminimo, 6) AS stockminimo,
                ROUND(asu.precio_compra, 6) AS precio_compra,
                ROUND(asu.ganacia_articulo, 6) AS ganacia_articulo,
                asu.tipo_ganacia,
                ROUND(asu.precio_venta, 6) AS precio_venta,
                ROUND(asu.precio_ventaNocturno, 6) AS precio_ventaNocturno,
                ROUND(asu.descuento_porcentaje, 6) AS descuento_porcentaje,
                ROUND(asu.precio_descuento, 6) AS precio_descuento,
                ROUND(asu.precio_rango1, 6) AS precio_rango1,
                ROUND(asu.precio_rango1_Dos, 6) AS precio_rango1_Dos,
                ROUND(asu.precio_rango2, 6) AS precio_rango2,
                ROUND(asu.precio_rango2_Dos, 6) AS precio_rango2_Dos,
                ROUND(asu.precio_rango3, 6) AS precio_rango3,
                ROUND(asu.precio_rango3_Dos, 6) AS precio_rango3_Dos,
                asu.nombre_01,
                ROUND(asu.stock_unidad, 6) AS stock_unidad,
                ROUND(asu.precio_unidad, 6) AS precio_unidad,
                asu.nombre_02,
                ...
                ROUND(asu.precio_paquete, 6) AS precio_paquete,
                asu.nombre_07,
                ROUND(asu.stock_07, 6) AS stock_07,
                ROUND(asu.precio_07, 6) AS precio_07,
                asu.nombre_08,
                ROUND(asu.stock_08, 6) AS stock_08,
                ROUND(asu.precio_08, 6) AS precio_08,
                asu.nombre_09,
                ROUND(asu.stock_09, 6) AS stock_09,
                ROUND(asu.precio_09, 6) AS precio_09,
                asu.nombre_10,
                ROUND(asu.stock_10, 6) AS stock_10,
                ROUND(asu.precio_10, 6) AS precio_10,
                asu.nombre_11,
                ROUND(asu.stock_11, 6) AS stock_11,
                ROUND(asu.precio_11, 6) AS precio_11,
                asu.nombre_12,
                ROUND(asu.stock_12, 6) AS stock_12,
                ROUND(asu.precio_12, 6) AS precio_12,
                asu.nombre_13,
                ROUND(asu.stock_13, 6) AS stock_13,
                ROUND(asu.precio_13, 6) AS precio_13,
                asu.nombre_14,
                ROUND(asu.stock_14, 6) AS stock_14,
                ROUND(asu.precio_14, 6) AS precio_14,
                asu.nombre_15,
                ROUND(asu.stock_15, 6) AS stock_15,
                ROUND(asu.precio_15, 6) AS precio_15,
                asu.nombre_16,
                ROUND(asu.stock_16, 6) AS stock_16,
                ROUND(asu.precio_16, 6) AS precio_16,
                asu.nombre_17,
                ROUND(asu.stock_17, 6) AS stock_17,
                ROUND(asu.precio_17, 6) AS precio_17,
                asu.nombre_18,
                ROUND(asu.stock_18, 6) AS stock_18,
                ROUND(asu.precio_18, 6) AS precio_18,
                asu.nombre_19,
                ROUND(asu.stock_19, 6) AS stock_19,
                ROUND(asu.precio_19, 6) AS precio_19,
                asu.nombre_20,
                ROUND(asu.stock_20, 6) AS stock_20,
                ROUND(asu.precio_20, 6) AS precio_20,
                ROUND(asu.stockmaximo, 6) AS stockmaximo,
                ROUND(asu.precio_rango1_Mecanico, 6) AS precio_rango1_Mecanico,
                ROUND(asu.precio_rango2_MecanicoDos, 6) AS precio_rango2_MecanicoDos,
                ROUND(asu.precio_rango3_MecanicoTres, 6) AS precio_rango3_MecanicoTres,
                ROUND(asu.precio_rango1_Distribuidor, 6) AS precio_rango1_Distribuidor,
                ROUND(asu.precio_rango2_DistribuidorDos, 6) AS precio_rango2_DistribuidorDos,
                ROUND(asu.precio_rango3_DistribuidorTres, 6) AS precio_rango3_DistribuidorTres,
                ROUND(asu.precio_rango1_Mayorista, 6) AS precio_rango1_Mayorista,
                ROUND(asu.precio_rango2_MayoristaDos, 6) AS precio_rango2_MayoristaDos,
                ROUND(asu.precio_rango3_MayoristaTres, 6) AS precio_rango3_MayoristaTres,
    ...
    public function listar($estadofiltro, $start, $length, $searchValue = "")
    {
        $searchFilter = "";
        if (!empty($searchValue)) {
            $searchFilter = " AND (a.nombre LIKE '%$searchValue%' OR a.codigo LIKE '%$searchValue%' OR c.nombre LIKE '%$searchValue%' OR sc.nombre LIKE '%$searchValue%') ";
        }
        ...
        $sql = "SELECT SQL_CALC_FOUND_ROWS
            ...
            LEFT JOIN (
                ...
                where   asu.idsucursal='" . $_SESSION["idsucursal"] . "' and   asu.condicion='$estadofiltro' $searchFilter
                LIMIT $start, $length ";
        return ejecutarConsulta($sql);
    }
    ...
    public function listarxGeneralAgrupado()
    {
        $sql = "SELECT 
                a.idarticulo,
                a.idcategoria, 
                ...
                a.ano_de,
                a.ano_a,
                sum(asu.stocksucursal) as stock, 
                a.stockminimo,
                a.descripcion,
                a.imagen,
                a.condicion,
